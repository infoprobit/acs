import { Children, ClosureComponent, Component, Vnode } from 'mithril';
        ...
        if (action === 'save') {
            ...
            if (object.precondition) {
                try {
                    ...
                    return void resolve({precondition: 'Precondition must be valid expression'});
                }
            }
...
export const component: ClosureComponent = (): Component => {
    return {
        view: (vnode) => {
            document.title = 'Pro ACS: Presets';
            ...
            for (let i = 0; i < attributes.length; i++) {
                ...
                if (!(attr.id === 'events' || attr.id === 'precondition' || attr.id === 'provision' || attr.id === 'provisionArgs'))
            ...
            const valueCallback = (attr, preset): Vnode => {
                if (attr.id === 'precondition') {
                    ...
                    if (preset['precondition'].length) {
                        devicesUrl += `?${m.buildQueryString({filter: preset['precondition']})}`;
                    }
                    ...
                    return m(
                        'a',
                        {
                            href: `#!/admin/provisions?${m.buildQueryString({filter: `Q("ID", "${preset['provision']}")`})}`,
                        },
                        preset['provision'],
                    );
                } else {
                ...
                attrs['actionsCallback'] = (selected: Set<string>): Children => {
                    return [
                        ...
                        m(
                            ...
                            {
                                title   : 'Delete Selected Preset(s)',
                                disabled: !selected.size,
                                onclick : (e) => {
                                    if (!confirm(`Deleting ${selected.size} Preset(s). Are you sure?`))
                                        return;

                                    e.redraw          = false;
                                    e.target.disabled = true;
                                    Promise
                                        .all(Array.from(selected).map((id) => store.deleteResource('presets', id)))
                                        .then((res) => {
                                            notifications.push('success', `${res.length} Preset(s) Deleted!`);
                                            store.setTimestamp(Date.now());
                                        })
                                        .catch((err) => {
                                            notifications.push('error', err.message);
            ...
            return [
                m('.pagetitle', m('h1', 'Listing Presets')),
