...
const attributes: { id: string; label: string; type?: string; options?: any; }[] = [
    {id: '_id', label: 'Name'},
    {
...
const getDownloadUrl = memoize((filter) => {
    ...
    return `api/files.csv?${m.buildQueryString({filter: stringify(filter), columns: JSON.stringify(cols)})}`;
});
...
export const component: ClosureComponent = (): Component => {
    return {
        view: (vnode) => {
                                ...
                                onclick: () => {
                                    ...
                                    cb                     = () => {
                                        ...
                                        return [
                                            m(
                                                'div.progress',
                                                m('div.progress-bar', {style: `width: ${Math.trunc(progress * 100)}%`}),
                                            ),
                                            comp,
                                        ];
                                    };
                                    overlay.open(cb, () => {
                                        if (comp.state['current']['modified'] && !confirm(
            ...
            return [
                m('.pagetitle', m('h1', 'Listing Files')),
