...
const getDownloadUrl = memoize((filter) => {
    ...
    return `api/provisions.csv?${m.buildQueryString({filter: stringify(filter), columns: JSON.stringify(cols)})}`;
});
...
export const component: ClosureComponent = (): Component => {
    return {
        view: (vnode) => {
            document.title = 'Provisions - ProACS';
                ...
                attrs['actionsCallback'] = (selected: Set<string>): Children => {
                    return [
                        ...
                        m(
                            ...
                            {
                                title   : 'Delete Selected Provision(s)',
                                disabled: !selected.size,
                                onclick : (e) => {
                                    if (!confirm(`Deleting ${selected.size} Provision(s). Are you sure?`))
                                        return;

                                    e.redraw          = false;
                                    e.target.disabled = true;
                                    Promise
                                        .all(Array.from(selected).map((id) => store.deleteResource('provisions', id)))
                                        .then((res) => {
                                            notifications.push('success', `${res.length} Provision(s) Deleted!`);
                                            store.setTimestamp(Date.now());
                                        })
                                        .catch((err) => {
                                            notifications.push('error', err.message);
