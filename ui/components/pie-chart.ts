...
function drawChart(chartData): Children {
    ...
    const total: number           = Array.from(Object.values(chartData.slices)).reduce(
        (a: number, s) => a + (s['count']['value'] || 0), 0,
    );
    const legend                  = [];
    const paths                   = [];
    const links                   = [];
    ...
    for (const slice of Object.values(slices)) {
        ...
        legend.push(
            m('.legend-line', [
                m('span.color', {style: `background-color: ${slice['color']} !important;`}),
                `${slice['label']}: `,
                m(
                    'a',
                    {href: `#!/devices/?${m.buildQueryString({filter: memoizedStringify(slice['filter'])})}`},
                    slice['count']['value'] || 0,
                ),
        ...
        if (percent > 0) {
            ...
            paths.push(m('path', {d: sketch, fill: slice['color']}));
            ...
            links.push(
                m(
                    'a',
                    {'xlink:href': `#!/devices/?${m.buildQueryString({filter: memoizedStringify(slice['filter'])})}`},
                    [
                        m('path', {d: sketch, 'fill-opacity': 0}),
                        m(
                            'text',
                            {x: percentageX, y: percentageY, 'dominant-baseline': 'middle', 'text-anchor': 'middle'},
                            `${(percent * 100).toFixed(2)}%`,
                        ),
                    ],
                ),
    ...
    return m(
        'loading', {queries: Object.values(chartData.slices).map((s) => s['count'])},
