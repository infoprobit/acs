        ...
        if (action === 'save') {
            ...
            if (object.filter) {
                try {
                    ...
                    return void resolve({filter: 'Filter must be valid expression'});
                }
            }
            ...
            if (object.validate) {
                try {
                    ...
                    return void resolve({validate: 'Validate must be valid expression'});
                }
            }
            ...
            store
                ...
                .then((exists) => {
                    ...
                    store
                        ...
                        .then(() => {
                            notifications.push('success', `Permission ${exists ? 'updated' : 'created'}`);
                            store.setTimestamp(Date.now());
                            resolve(null);
                        })
                        .catch(reject);
...
const getDownloadUrl = memoize((filter) => {
    ...
    return `api/permissions.csv?${m.buildQueryString({filter: stringify(filter), columns: JSON.stringify(cols)})}`;
});
...
export const component: ClosureComponent = (): Component => {
    return {
        view: (vnode) => {
            document.title = 'Pro ACS: Permissions';
                                        ...
                                        Object.assign(
                                            {
                                                base         : permission,
                                                oncreate     : (_vnode) => {
                                                    _vnode.dom.querySelector('input[name=\'role\']').disabled      = true;
                                                    _vnode.dom.querySelector('select[name=\'access\']').disabled   = true;
                                                    _vnode.dom.querySelector('select[name=\'resource\']').disabled = true;
                                                },
                    ...
                    return [
                        ...
                        m(
                            'button.btn.btn-outline-danger',
                            {
                                ...
                                onclick : (e) => {
                                    if (!confirm(`Deleting ${selected.size} Permission(s). Are you sure?`))
                                        return;
                                    e.redraw          = false;
                                    e.target.disabled = true;
                                    Promise
                                        .all(Array.from(selected).map((id) => store.deleteResource('permissions', id)))
                                        .then((res) => {
                                            notifications.push('success', `${res.length} Permission(s) Deleted!`);
                                            store.setTimestamp(Date.now());
                                        })
                                        .catch((err) => {
                                            notifications.push('error', err.message);
