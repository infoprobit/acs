...
const getDownloadUrl = memoize((filter) => {
    ...
    return `api/faults.csv?${m.buildQueryString({filter: stringify(filter), columns: JSON.stringify(cols)})}`;
});
...
export const component: ClosureComponent = (): Component => {
    return {
        view: (vnode) => {
                    ...
                    return m(
                        'button.btn.btn-outline-danger',
                        {
                            ...
                            onclick : (e) => {
                                ...
                                Promise
                                    .all(Array.from(selected).map((id) => store.deleteResource('faults', id)))
                                    .then((res) => {
            ...
            return [
                m('.pagetitle', m('h1', 'Listing Faults')),
