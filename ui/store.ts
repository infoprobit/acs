...
import { and, evaluate, or } from '../lib/common/expression/util.ts';
...
import { bookmarkToExpression, paginate, toBookmark } from '../lib/common/expression/pagination.ts';
...
export async function xhrRequest(options: { url: string } & m.RequestOptions<unknown>): Promise<any> {
...
function compareFunction(sort: { [param: string]: number; }): (a: any, b: any) => number {
    return (a, b) => {
...
export function postTasks(deviceId: string, tasks: QueueTask[]): Promise<string> {
    const tasks2: Task[] = [];
...
export function deleteResource(resourceType: string, id: string): Promise<void> {
    return xhrRequest({method: 'DELETE', url: `api/${resourceType}/${encodeURIComponent(id)}`});
}
...
export function resourceExists(resource: string, id: string): Promise<number> {
    ...
    return xhrRequest({
                          method    : 'HEAD',
                          url       : `api/${resource}/?` + m.buildQueryString({filter: memoizedStringify(filter)}),
                          extract   : (xhr) => {
                              if (xhr.status === 403) throw new Error('Not authorized');
                              if (!xhr.status) throw new Error('Server is unreachable');
                              else if (xhr.status !== 200)
...
export function evaluateExpression(exp: Expression, obj: Record<string, unknown>): Expression {
    if (!Array.isArray(exp)) return exp;
    return memoizedEvaluate(exp, obj, fulfillTimestamp + clockSkew);
}

export function changePassword(username: string, newPassword: string, authPassword?: string): Promise<void> {
...
export function logOut(): Promise<void> {
    return xhrRequest({method: 'POST', url: 'logout'});
}
