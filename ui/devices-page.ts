...
const getDownloadUrl = memoize((filter, indexParameters) => {
    ...
    return `api/devices.csv?${m.buildQueryString({filter: stringify(filter), columns: JSON.stringify(columns)})}`;
});
...
export function init(args: Record<string, unknown>): Promise<Record<string, unknown>> {
    return new Promise((resolve, reject) => {
        ...
        if (!indexParameters.length) {
            indexParameters.push({label: 'ID', parameter: ['PARAM', 'DeviceID.ID']});
        }
...
export const component: ClosureComponent = (): Component => {
    return {
        view: (vnode) => {
            ...
            return [
                m('.pagetitle', m('h1', 'Listing Devices')),
