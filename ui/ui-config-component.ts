...
const component: ClosureComponent<Attrs> = () => {
    return {
        view: (vnode) => {
            ...
            const form = [m('div', {class: 'row mb-12 code'}, m(codeEditorComponent, attrs))];
            form.push(m('.modal-footer.actions-bar', m('button.btn.btn-primary', {type: 'submit'}, 'Save')))

            const children = [
                m('.modal-header', m('h5.modal-title', `Editing ${name}`)),
                m('.modal-body', m('form', {
                    onsubmit: (e: any) => {
                        e.redraw = false;
                        e.preventDefault();
