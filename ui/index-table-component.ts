...
function renderTable(
    ...
    if (buttons.length) {
        ...
        labels.push(m('th', {scope: 'col'}, selectAll));
    }
    ...
    for (let i = 0; i < attributes.length; i++) {
        ...
        if (!sortAttributes.hasOwnProperty(i)) {
            labels.push(m('th', {scope: 'col'}, label));
            continue;
        }
        ...
        labels.push(m('th', {scope: 'col'}, [label, sortable]));
    }

    if (recordActionsCallback !== undefined) {
        labels.push(m('th', ''));
    }
    ...
    if (total != null) footerElements.push(m('span.p-2', `Showing ${records.length} of ${total} entries`));
    ...
    if (downloadUrl && data.length > 0) {
        footerElements.push(
            m('span.p-1', m('a.btn.btn-secondary', {href: downloadUrl, download: ''}, 'Download')),
        );
    }
