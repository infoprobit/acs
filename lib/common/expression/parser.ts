...
export async function mapAsync(exp: Expression, callback: (e: Expression) => Promise<Expression>): Promise<Expression> {
    if (!Array.isArray(exp)) return callback(exp);
...
export function stringify(exp: Expression, level = 0): string {
    ...
    if (op === 'FUNC') {
        return wrap(`${exp[1]}(${exp.slice(2).map((e) => stringify(e)).join(', ')})`);
    } else if (op === 'PARAM') {
        if (typeof exp[1] === 'string') {
            return wrap(exp[1]);
        } else if (Array.isArray(exp[1]) && exp[1][0] === '||') {
            return wrap(exp[1].slice(1).map((p) => {
        ...
        if (exp[3]) {
            return wrap(`${stringify(exp[1], opLevels[op])} ${op} ${stringify(exp[2], opLevels[op])} ESCAPE ${stringify(
                exp[3],
                opLevels[op],
            )}`);
        } else {
