...
import { and, evaluate, extractParams } from '../common/expression/util.ts';
import { parse } from '../common/expression/parser.ts';
import * as logger from '../logger.ts';
import { getConfig } from './local-cache.ts';
...
router.get('/aggregation/:resource', async (ctx) => {
    const {resource}             = ctx.params;
    const authorizer: Authorizer = ctx.state.authorizer;
    const log                    = {
        message: `Aggregation query on ${resource}`,
        context: ctx,
    };
    ...
    const groupBy    = ctx.request.query.groupBy || '_id'; // default group by id
    const logDetails = {groupBy, filter: filter};

    try {
        ctx.body = await db.aggregate(resource, {filter, groupBy});
        logger.accessInfo({...log, ...logDetails});
    } catch (error) {
        logger.accessError({...log, error, ...logDetails});
        ctx.status = 500;
        ctx.body   = 'Server error during aggregation';
    }
});
