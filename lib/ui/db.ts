...
function processDeviceProjection(projection: Record<string, 1>): Record<string, 1> {
...
function processDeviceSort(sort: Record<string, number>): Record<string, number> {
    if (!sort) return sort;
    const s = {};
...
export function flattenDevice(device: Record<string, unknown>): FlatDevice {
    function recursive(input, root: string, output: FlatDevice, timestamp: number): void {
...
function flattenPreset(preset: Record<string, unknown>): Record<string, unknown> {
    ...
    if (p.precondition) {
        try {
            ...
            p.precondition = (p.precondition as string).length ? stringify(p.precondition as Expression) : '';
        }
    }
    ...
    if (
        ...
        p.provisionArgs = provision.args ? provision.args.map((a) => stringify(a)).join(', ') : '';
    }
...
export async function updateDeviceTags(deviceId: string, tags: Record<string, boolean>): Promise<void> {
    const add  = [];
    const pull = [];
...
export async function putPreset(id: string, object: Record<string, unknown>): Promise<void> {
...
export async function putProvision(id: string, object: { script: string }): Promise<void> {
...
export async function putVirtualParameter(id: string, object: { script: string }): Promise<void> {
    if (!object.script) object.script = '';
    try {
        new Script(`"use strict";(function(){\n${object.script}\n})();`, {
            filename  : id,
...
export async function putConfig(id: string, object: WithoutId<MongoTypes.Config>): Promise<void> {
...
export async function putPermission(id: string, object: WithoutId<MongoTypes.Permission>): Promise<void> {
...
export async function putUser(id: string, object: Partial<WithoutId<MongoTypes.User>>): Promise<void> {
...
export function putFile(filename: string, metadata: Record<string, string>, contentStream: Readable): Promise<void> {
...
export async function aggregate(resource: string, {filter, groupBy}) {
    const collection = collections[resource] as Collection<any>;
    const pipeline   = [];
